...
const backgroundColors = ["#53c6a2", "#fdd762", "#9261d3", "#43dce7", "#ffcc5a", "#ea4398", "#4a5de1", "#e95555", "#7eda54", "#f9b647"];
...
export const createUser = userData => dispatch => {
    ...
    return QB.users
        ...
        .catch(e => {
            showError("Falha ao criar um novo utilizador", e.message);
...
export const getUsers = (query = defaultQuery) => (dispatch, getState) => {
    const {
        users: { page, perPage, users: savedUsers }
    } = getState();
    const filterUserId = query.filter && query.filter.field === QB.users.USERS_FILTER.FIELD.ID && query.filter.value && query.filter.value;
    if (filterUserId) {
    ...
    return QB.users
        ...
        .then(response => {
            const usersWithColor = response.users.map(user => {
                ...
                const color = savedUser && savedUser.color ? savedUser.color : getRandomColor();
                return { ...user, color };
            });
            dispatch(
                usersGetSuccess({
        ...
        .catch(e => {
            showError("Falha ao obter utilizadores", e.message);
