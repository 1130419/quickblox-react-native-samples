...
import { dialogEditSuccess, dialogUnreadCountIncrement } from "../actionCreators";
...
export function subscribe(dispatcher, getState) {
    ...
    QBConnectionEvents.forEach(eventName => {
        subscriptions.push(emitter.addListener(eventName, connectionEventHandler));
    });
    subscriptions.push(emitter.addListener(QB.chat.EVENT_TYPE.RECEIVED_NEW_MESSAGE, receivedNewMessage));
    subscriptions.push(emitter.addListener(QB.chat.EVENT_TYPE.MESSAGE_DELIVERED, messageStatusHandler));
    subscriptions.push(emitter.addListener(QB.chat.EVENT_TYPE.MESSAGE_READ, messageStatusHandler));
    subscriptions.push(emitter.addListener(QB.chat.EVENT_TYPE.RECEIVED_SYSTEM_MESSAGE, systemMessageHandler));
    subscriptions.push(emitter.addListener(QB.chat.EVENT_TYPE.USER_IS_TYPING, userTypingHandler));
    subscriptions.push(emitter.addListener(QB.chat.EVENT_TYPE.USER_STOPPED_TYPING, userTypingHandler));
    dispatch(getDialogs());
    const route = Navigation.getCurrentRoute();
    const { routeName, params = { dialog: {} } } = route;
