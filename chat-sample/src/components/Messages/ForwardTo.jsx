...
export default class ForwardTo extends React.Component {
        ...
        return {
            headerTitle: (
                <View style={styles.titleView}>
                    <Text style={styles.titleText}>Para a frente</Text>
                    <Text style={styles.titleSmallText}>{selected.length} conversa(s)</Text>
                </View>
            ),
            headerRight: (
                <TouchableOpacity disabled={selected.length === 0} onPress={sendHandler} style={styles.headerButton}>
    ...
    forwardMessage = () => {
        ...
        const promises = selected.map(dialogId =>
            sendMessage({
                attachments,
                body,
                dialogId,
