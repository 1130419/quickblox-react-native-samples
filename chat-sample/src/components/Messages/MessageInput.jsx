...
import { ActivityIndicator, StyleSheet, Image, Text, TextInput, TouchableOpacity, View } from "react-native";
...
export default class MessageInput extends React.Component {
    ...
    fileSelected = response => {
        ...
        if (response.error) {
            return showError("Erro", response.error);
        }
        if (response.uri) {
            let message;
            if (response.fileName) {
                ...
                if (response.path.lastIndexOf("/") > -1) {
                    const afterLastSlash = response.path.substring(response.path.lastIndexOf("/") + 1);
    ...
    sendIsTyping = () => QB.chat.sendIsTyping({ dialogId: this.props.dialogId }).catch(e => console.warn(e.message));

    sendStoppedTyping = () => QB.chat.sendStoppedTyping({ dialogId: this.props.dialogId }).catch(e => console.warn(e.message));
    ...
    sendMessage = () => {
        ...
        fileUpload.then(result => {
            if (result && result.error) {
                return showError("Falha ao carregar ficheiro", result.error);
            }
    ...
    render() {
        return (
            <View style={styles.inputView}>
                {this.state.fileUrl ? (
                    <TouchableOpacity disabled={this.props.isSending} onPress={this.clearFile} style={styles.attachClearBtn}>
                        <Text style={styles.attachClearText}>&times;</Text>
                    </TouchableOpacity>
                ) : (
                    <AttachButton disabled={this.props.isSending} onAttachment={this.fileSelected} />
                )}
                <TextInput
                ...
                />
                <TouchableOpacity disabled={this.props.isSending || !this.state.message.trim()} onPress={this.sendMessage} style={styles.sendBtn}>
                    {this.props.isSending ? <ActivityIndicator color={colors.primary} size={28} /> : <Image source={SEND} style={styles.sendBtnIcon} />}
                </TouchableOpacity>
            </View>
        );
    }
