...
import { Animated, BackHandler, Dimensions, Modal, StyleSheet, Text, TouchableOpacity, TouchableWithoutFeedback, UIManager } from "react-native";
...
export default class LongPressMenu extends React.Component {
    ...
    constructor(props) {
        ...
        this.menuItems = [
            <TouchableOpacity key="forward" onPress={this.forwardPressHandler} style={styles.menuItemBtn}>
                <Text style={styles.menuItemText}>Encaminhar</Text>
            </TouchableOpacity>
        ];
        if (dialogType === QB.chat.DIALOG_TYPE.GROUP_CHAT) {
            this.menuItems.push(
                <TouchableOpacity key="delivered" onPress={this.deliveredPressHandler} style={styles.menuItemBtn}>
                    <Text style={styles.menuItemText}>Entregue a...</Text>
                </TouchableOpacity>,
                <TouchableOpacity key="viewed" onPress={this.viewedPressHandler} style={styles.menuItemBtn}>
                    <Text style={styles.menuItemText}>Visto por...</Text>
                </TouchableOpacity>
            );
        }
    ...
    componentDidMount() {
        this.backListener = BackHandler.addEventListener("hardwareBackPress", this.hideMenu);
    }
        ...
        return (
            <React.Fragment>
                <TouchableWithoutFeedback onLongPress={this.showMenu}>
                    {React.cloneElement(this.props.children, { onLongPress: this.showMenu })}
                </TouchableWithoutFeedback>
                <Modal animationType="none" onRequestClose={this.hideMenu} supportedOrientations={["landscape", "portrait"]} transparent visible={show}>
                    <Animated.View onStartShouldSetResponder={() => true} onResponderRelease={this.hideMenu} style={[styles.backdrop, { opacity: this.scale }]}>
