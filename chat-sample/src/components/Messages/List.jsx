...
export default class MessagesList extends React.PureComponent {
    ...
    renderLoadingIndicator = () => (this.props.loading ? <ActivityIndicator color={colors.primary} size={30} style={{ padding: 10 }} /> : null);
    ...
    listEmptyComponent = () =>
        this.props.loading ? null : (
            <View style={styles.noMessagesView}>
                <Text>Ainda não há mensagens nesta conversa</Text>
            </View>
        );
    ...
    renderSectionHeader = ({ section }) => (
        <View style={styles.sectionHeaderView}>
            <View style={styles.sectionHeaderTextView}>
                <Text style={styles.sectionHeaderText}>{this.getDateString(section.title)}</Text>
            </View>
        </View>
    );
        ...
        if (unreadMessagesCount) {
            viewableItems
                ...
                .forEach(({ isViewable, item: message }) => {
                    ...
                    const shouldMarkAsRead = isViewable && readIds.indexOf(currentUser.id) === -1;
                    if (shouldMarkAsRead) {
                        markAsRead(message);
                    }
                });
    ...
    render() {
        ...
        return (
            <SectionList
                ...
                ListHeaderComponent={<TypingIndicator dialogId={dialogId} style={{ padding: 10 }} />}
