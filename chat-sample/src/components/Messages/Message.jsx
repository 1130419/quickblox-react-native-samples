...
const Shadow = Platform.select({
    android: () => <Image resizeMode="stretch" source={SHADOW} style={styles.shadowImg} />,
    ios: () => null
});
...
export default class Message extends React.Component {
    ...
    componentDidMount() {
        const { getPrivateUrl, getUsers, imageUrl, message, sender } = this.props;
    ...
    shouldComponentUpdate(nextProps) {
        ...
        const { deliveredIds: _deliveredIds = [], readIds: _readIds = [] } = nextProps.message;
        return (
            deliveredIds.length !== _deliveredIds.length || imageUrl !== nextProps.imageUrl || readIds.length !== _readIds.length || sender !== nextProps.sender
        );
    }
    ...
    getMessageBody = (messageIsMine = false) => {
        ...
        const withMediaAttachment = this.hasImageAttachment || this.hasVideoAttachment;
        const _styles = messageIsMine
            ? {
                  bodyView: withMediaAttachment ? styles.myMessageBodyMedia : styles.myMessageBodyView,
                  text: styles.myMessageBodyText
              }
            : {
                  bodyView: withMediaAttachment ? styles.messageBodyMedia : styles.messageBodyView,
                  text: styles.messageBodyText
              };
        const forwarded = properties && properties.originDialogName && properties.originDialogName.length;
        ...
        if (this.hasImageAttachment) {
            msgBody = onLongPress => <RemoteImage onLongPress={onLongPress} resizeMode="cover" source={{ uri: imageUrl }} style={styles.attachment} />;
        } else if (this.hasVideoAttachment) {
            msgBody = onLongPress => <RemoteVideo onLongPress={onLongPress} resizeMode="cover" source={{ uri: imageUrl }} style={styles.attachment} />;
        } else {
            msgBody = () => (
        ...
        const forwardedText = forwarded ? (
            <View pointerEvents="none">
                <Text numberOfLines={1} style={forwardedTextStyle}>
                    Encaminhado de&nbsp;
                    <Text style={forwardedBoldTextStyle}>{properties.originDialogName}</Text>
                </Text>
            </View>
        ) : null;
        const MessageBody = ({ onLongPress }) => (
            <TouchableWithoutFeedback onLongPress={withMediaAttachment ? null : onLongPress}>
    ...
    getMessage = () => {
        ...
        const sentBy = sender ? sender.fullName || sender.login || sender.email : senderId.toString();
        ...
        const withMediaAttachment = this.hasImageAttachment || this.hasVideoAttachment;
        const metaViewStyle = withMediaAttachment ? styles.mediaMessageMeta : styles.messageMeta;
        return (
            <View style={styles.messageView}>
                {privateChat ? null : (
                    <View style={[styles.senderCircleView, { backgroundColor: circleBackground }]}>
            ...
            <View style={styles.myMessageView}>
                <View style={styles.messageContent}>
                    <View style={metaViewStyle}>
                        ...
                        <View style={{ flex: 1 }} />
                        {delivered ? <Image source={CHECKMARK_DOUBLE} style={checkmarkStyle} /> : <Image source={CHECKMARK} style={checkmarkStyle} />}
                        <Text style={styles.messageSentAt}>{sentAt}</Text>
                    </View>
                    {this.getMessageBody(true)}
                    {this.hasImageAttachment || this.hasVideoAttachment ? null : <Shadow />}
                </View>
            </View>
        );
    };
            ...
            return (
                <View style={styles.messageView}>
                    <View style={styles.systemMessage}>
                        <Text style={styles.messageSentAt}>{body}</Text>
                    </View>
                </View>
            );
        }
