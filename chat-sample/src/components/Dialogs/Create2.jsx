...
export default class DialogsCreate2 extends React.PureComponent {
        ...
        return {
            headerTitle: (
                <View style={styles.titleView}>
                    <Text style={styles.titleText}>Nova Conversa</Text>
                </View>
            ),
            headerRight: (
                <TouchableOpacity onPress={finishHandler} style={styles.headerButton}>
                    <Text style={styles.headerButtonText}>Terminar</Text>
                </TouchableOpacity>
            )
        };
    };
    ...
    submit = ({ name }) => {
        const { createDialog, currentUser, navigation, occupantsIds, sendMessage, users } = this.props;
        return createDialog({ name, occupantsIds }).then(result => {
            if (result.error) {
                ...
                const body = myName + " criou uma nova conversa com: " + opponentsNames.join(", ");
                sendMessage({
    ...
    renderForm = ({ handleSubmit }) => {
        ...
        return (
            <SafeAreaView style={styles.safeArea}>
                <KeyboardAvoidingView behavior={Platform.select({ ios: "padding" })} style={localStyles.contentView}>
                    <View style={localStyles.formControlView}>
                        <Text style={localStyles.label}>Nome da conversa</Text>
                        <Field
    ...
    render() {
        return <Form render={this.renderForm} onSubmit={this.submit} validate={this.validate} />;
    }
}
