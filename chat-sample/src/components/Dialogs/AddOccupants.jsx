...
export default class AddOccupants extends React.Component {
        ...
        return {
            headerTitle: (
                <View style={styles.titleView}>
                    <Text style={styles.titleText}>Adicionar pessoas</Text>
                    <Text style={styles.titleSmallText}>{selected.length} utilizadores selecionados</Text>
                </View>
            ),
            headerRight: (
                <TouchableOpacity disabled={selected.length === 0} onPress={onDonePress} style={styles.headerButton}>
                    <Text style={styles.headerButtonText}>Feito</Text>
                </TouchableOpacity>
            )
        };
    };
    ...
    updateDialog = () => {
        const { currentUser, navigation, selected = [], sendMessage, updateDialog, users } = this.props;
        ...
        if (dialog && selected.length && updateDialog) {
            updateDialog({
                ...
                if (result && result.payload) {
                    ...
                    const body = `${myName} adicionou ${newUsersNames.join(", ")} à conversa`;
                    sendMessage({
                        dialogId: dialog.id,
                        body,
