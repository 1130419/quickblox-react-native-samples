...
class Dialogs extends React.Component {
    ...
    static navigationOptions = ({ navigation }) => {
        ...
        return deleteMode
            ? {
                  headerLeft: (
                      <TouchableOpacity onPress={cancelDelete} style={styles.headerButton}>
                          <Text style={styles.headerButtonText}>Cancelar</Text>
                      </TouchableOpacity>
                  ),
                  headerTitle: (
                      <View style={styles.titleView}>
                          <Text style={styles.titleText}>Remover Conversas</Text>
                          <Text style={styles.titleSmallText}>{selected.length} conversas selecionadas</Text>
                      </View>
                  ),
                  headerRight: (
                      <TouchableOpacity onPress={deleteDialogs} style={styles.headerButton}>
                          <Text style={styles.headerButtonText}>Remover</Text>
                      </TouchableOpacity>
                  )
              }
            : {
                  headerLeft: <HeaderButton onPress={handleLogout} imageSource={EXIT} />,
                  headerTitle: (
                      <View style={styles.titleView}>
                  ...
                  headerRight: (
                      <React.Fragment>
                          <HeaderButton imageSource={INFO} onPress={() => navigation.navigate("Info")} />
                          <HeaderButton imageSource={ADD} onPress={() => navigation.navigate("DialogsCreate1")} />
                      </React.Fragment>
                  )
              };
    };
