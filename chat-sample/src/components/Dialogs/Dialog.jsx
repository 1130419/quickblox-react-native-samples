...
export default class Dialog extends React.PureComponent {
    ...
    getLastMessageDateString = () => {
        ...
        if (lastMessageDate.getFullYear() === now.getFullYear() && lastMessageDate.getMonth() === now.getMonth()) {
    ...
    render() {
        ...
        const dialogBtnStyle = isSelected ? [styles.dialogBtn, styles.dialogBtnSelected] : styles.dialogBtn;
        ...
        return (
            <TouchableOpacity onLongPress={this.onLongPress} onPress={this.onPress} style={dialogBtnStyle}>
                {dialog.photo ? (
                    <Image resizeMode="center" source={{ uri: dialog.photo }} style={styles.dialogCircle} borderRadius={80} />
                ) : (
                ...
                <View style={styles.dialogRightView}>
                    {selectable ? (
                        ...
                        <React.Fragment>
                            <Text style={styles.dialogLastMessageDate}>{this.getLastMessageDateString()}</Text>
                            {dialog.unreadMessagesCount ? (
                                <View style={styles.unreadMsgBadge}>
                                    <Text style={styles.unreadMsgText}>{dialog.unreadMessagesCount > 100 ? "99+" : dialog.unreadMessagesCount}</Text>
                                </View>
                            ) : null}
                        </React.Fragment>
                    )}
